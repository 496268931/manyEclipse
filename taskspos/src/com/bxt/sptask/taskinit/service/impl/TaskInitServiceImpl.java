package com.bxt.sptask.taskinit.service.impl;
...
import com.bxt.sptask.taskinit.dao.impl.TaskInitDaoImpl;
...
public class TaskInitServiceImpl implements TaskInitService {
	 ...
	 */
	@Override
	public void initTaskHashmap(WebApplicationContext wact) {
		//初始化任务
...
//			}
//		}
		List<TaskVo> taskgroup = taskDao.getTaskGroupInfo(wact);
		int xh = 0;
		...
		if(taskgroup != null){
			for(TaskVo taskvo:taskgroup){
				taskEveryGroup = taskDao.getEveryGroupTaskID(wact,taskvo);
...
//					}
	 ...
	 */
	@Override
	public void waitTaskKeyHandle(WebApplicationContext wact) {
				...
				try{
					...
					if(StaticMapParams.listWaitHandleKey.size() > 0){
						for(String sTaskKey : StaticMapParams.listWaitHandleKey){
							...
							List<String> listTaskID = new ArrayList<String>();
							List<TaskVo> taskEveryGroup = taskDao.getEveryGroupTaskID(wact, taskvo);
							for(TaskVo taskevery:taskEveryGroup){
								listTaskID.add(taskevery.getId());
							}
	 ...
	 */
	@Override
	public void waitTaskKeyRead(WebApplicationContext wact) {
	...
	public void handleOutTimeTask(WebApplicationContext wact) {
		TaskInitDao taskDao = new TaskInitDaoImpl();
		taskDao.handleOutTimeTask(wact);
