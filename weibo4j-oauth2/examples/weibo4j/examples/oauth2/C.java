...
public class C {
    private static final String POST_URI = "http://login.sina.com.cn/sso/login.php?client=ssologin.js(v1.4.2)";
...
//        ; 
    ...
    public static String Base64Encode(String account) { 
        ...
        try { 
            userName = encoder.encode(URLEncoder.encode(account,"UTF-8").getBytes());    
    ...
    private JSONObject prelogin() throws HttpException, IOException {
        String prelogin_url = "http://login.sina.com.cn/sso/prelogin.php?entry=sso&" +
            "callback=sinaSSOController.preloginCallBack&su=" +
            username.replace("@", "@") +
            "&rsakt=mod&client=ssologin.js(v1.4.2)" +
            System.currentTimeMillis();
       
        HttpMethod method = new GetMethod(prelogin_url);
    ...
    private String passwordEncryptRSA(String pubkey, String password, String servertime,
            String nonce) throws ScriptException, NoSuchMethodException{
       ...
       //原代码 se.eval_r(getJs());
        ...
        if(se instanceof Invocable) {
            ...
            pass = (String) invoke.invokeFunction("getpass", password, servertime, nonce, pubkey);
        }
        return pass;
    }
    ...
    public void login() throws Exception {
        ...
        post.addParameter("nonce", params.getString("nonce"));
        post.addParameter("prelt", "130");
        post.addParameter("pwencode", "rsa2");
        post.addParameter("returntype", "META");
        post.addParameter("rsakv", params.getString("rsakv"));
        post.addParameter("savestate", "7");
        post.addParameter("servertime", params.getString("servertime"));
        post.addParameter("service", "miniblog");
        post.addParameter("ssosimplelogin", "1");
        post.addParameter("sp",
                this.passwordEncryptRSA(params.getString("pubkey"),
                        password,
                        params.getString("servertime"),
                        params.getString("nonce")));
        post.addParameter("su", Base64Encode(username));
        post.addParameter("url", "http://login.sina.com.cn/sso/login.php?client=ssologin.js(v1.4.18)");
        //post.addParameter("url", "https://api.weibo.com/oauth2/authorize?client_id=1651601463&redirect_uri=http%3A%2F%2Fsns.whalecloud.com%2Fsina%2Fcallback%3Fimei%3Dc1cbc99be4cf339b14869bb91d963cf%26appkey%3D541cf68dfd98c51895027d3c%26key%3D1651601463%26secret%3D84502c2a408c8de5a1c6b7f76d574a84%26pcv%3D2.0&display=mobile&forcelogin=true&fc=umeng&response_type=code&scope=all&with_offical_account=1&pcv=2.0");
        ...
        String redirect_uri = "";
        if(m.find())    redirect_uri = m.group(1);System.out.println(redirect_uri);
        // retcode=0 说明正常，继续访问重定向url
        if(redirect_uri.indexOf("retcode=0") > -1) {
            GetMethod get = new GetMethod(redirect_uri);
            client.executeMethod(get);
            Header[] hdrs = get.getResponseHeaders("Set-Cookie");
